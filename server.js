...
const app = express();
// Railway menjalankan Express di belakang reverse proxy.
// Trust satu hop proxy agar req.ip/X-Forwarded-For kompatibel dengan rate limiter.
...
function ensureSeed(){
  ...
  // Sinkronkan kredensial admin dari environment setiap startup.
  // Ini mencegah DB lama menyimpan password/nomor email demo yang sudah tidak sama.
  admin.email=adminEmail;
  admin.passwordHash=hash(adminPass);
  admin.status='ACTIVE';
...
app.get('/api/diagnostics',(req,res)=>res.json({ok:true,data:{adminConfigured:Boolean(process.env.ADMIN_EMAIL&&process.env.ADMIN_PASSWORD),randomOtpEnabled:true,devOtpLogEnabled:process.env.DEV_OTP_LOG==='true',nodeEnv:process.env.NODE_ENV||'development'}}));
...
app.use((err,req,res,next)=>{ if(err && err.type==='request.aborted'){ console.warn('[IKSN] request aborted before body completed'); if(!res.headersSent) return res.status(400).json({ok:false,error:{code:'REQUEST_ABORTED',message:'Permintaan terputus sebelum selesai. Silakan coba lagi.'}}); return; } if(err){ console.error(err); if(!res.headersSent) return res.status(400).json({ok:false,error:{code:'BAD_REQUEST',message:'Permintaan tidak dapat diproses.'}}); } next(); });
...
function createOtp(principalId,channel='personnel'){
  // OTP selalu acak; tidak ada fixed OTP untuk admin maupun demo personel.
  const code=String(crypto.randomInt(100000,1000000));
  db.otpCodes=db.otpCodes.filter(x=>x.principalId!==principalId || x.channel!==channel || x.used || new Date(x.expiresAt).getTime()<Date.now());
  const item={id:uid('OTP'),principalId,code,channel,createdAt:iso(),expiresAt:new Date(Date.now()+OTP_TTL_MS).toISOString(),used:false,attempts:0};
  db.otpCodes.push(item);
  writeDb(db);
  if(process.env.DEV_OTP_LOG==='true') console.log(`[IKSN OTP] ${iso()} ${channel} ${principalId}: ${code} (expires +5m)`);
  return item;
}
...
function input(req,key,header){ const h=req.headers[header.toLowerCase()]; return h!==undefined ? escapeNull(h) : escapeNull(req.body?.[key]); }
...
app.post('/api/auth/admin/request-otp',(req,res)=>{
  const email=input(req,'email','x-iksn-email').trim().toLowerCase(); const password=input(req,'password','x-iksn-password');
  ...
  res.json({ok:true,data:{challengeId:otp.id,expiresAt:otp.expiresAt}});
});
app.post('/api/auth/admin/verify-otp',(req,res)=>{
  const email=input(req,'email','x-iksn-email').trim().toLowerCase();const code=String(input(req,'otp','x-iksn-otp')).trim();const admin=db.admins.find(a=>a.email===email);
...
app.post('/api/auth/personnel/request-otp',(req,res)=>{
  const email=input(req,'email','x-iksn-email').trim().toLowerCase();const user=db.users.find(u=>u.email===email&&u.status==='Active');
  ...
  const otp=createOtp(user.id,'personnel');audit(user.id,'PERSONNEL','OTP_REQUESTED',p.id,'SUCCESS',{});res.json({ok:true,data:{challengeId:otp.id,expiresAt:otp.expiresAt}});
});
app.post('/api/auth/personnel/verify-otp',(req,res)=>{
  const email=input(req,'email','x-iksn-email').trim().toLowerCase();const code=String(input(req,'otp','x-iksn-otp')).trim();const user=db.users.find(u=>u.email===email&&u.status==='Active');
  if(!user)return res.status(401).json({ok:false,error:{code:'INVALID_CREDENTIALS',message:'Credentials tidak valid.'}});
...
app.post('/api/auth/personnel/verify-credentials',requireAuth,requireRole('PERSONNEL'),(req,res)=>{
  const serial=input(req,'serial','x-iksn-serial').trim();const password=input(req,'password','x-iksn-password');const p=db.personnel.find(x=>x.id===req.session.personnelId&&!x.deleted);
